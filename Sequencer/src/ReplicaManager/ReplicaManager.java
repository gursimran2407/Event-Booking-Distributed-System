...
public class ReplicaManager {
    ...
    private static void checkAndStartTheReplicas() {
        ...
        for (String serverCode : replicaPorts) {
            ...
            if (!response.equals(CommonUtils.ALIVE)) {
                ...
                try (DatagramSocket socket = new DatagramSocket()) {
                    InetAddress host = InetAddress.getByName(CommonUtils.SEQUENCER_HOSTNAME); //Varies per Replica
                    ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
                    ObjectOutput objectOutput = new ObjectOutputStream(byteStream);
    ...
    private static void startTheServer(String serverCode) {
        ...
        switch (serverCode) {
            case CommonUtils.MONTREAL:
                MontrealServer.main(stringArray);
    ...
    private static int getServerPort(String serverCode) {
        switch (serverCode) {
            case CommonUtils.MONTREAL:
                return CommonUtils.REPLICA_MONTREAL_SERVER_PORT;
            case CommonUtils.OTTAWA:
                return CommonUtils.REPLICA_OTTAWA_SERVER_PORT;
            case CommonUtils.TORONTO:
                return CommonUtils.REPLICA_TORONTO_SERVER_PORT;
        }
        return 0;
    }
